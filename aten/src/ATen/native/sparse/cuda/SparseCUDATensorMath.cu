...
//LUIS_MODS
SparseTensor& luis_add_out_sparse_cuda(const SparseTensor& t, const SparseTensor& src, const Scalar& value, SparseTensor& r_) {
  if (!t.is_sparse()) {
    return add_out_dense_sparse_cuda(r_, t, src, value);
  }
  ...
  TORCH_CHECK(t.is_cuda(), "luis_add: expected 'self' to be CUDA, but got CPU");
  TORCH_CHECK(src.is_cuda(), "luis_add: expected 'other' to be CUDA, but got CPU");
  TORCH_CHECK(r_.is_cuda(), "luis_add: expected 'out' to be CUDA, but got CPU");
  ...
  TORCH_CHECK(t.sizes().equals(src.sizes()), "luis_add: expected 'self' and 'other' to have same size, but ", t.sizes(), " != ", src.sizes());
  ...
  AT_DISPATCH_ALL_TYPES_AND_COMPLEX_AND2(
    at::ScalarType::Half, at::ScalarType::BFloat16, commonDtype, "luis_add_out_sparse_cuda", [&] {
      if (value.to<scalar_t>() != scalar_t(1)) {
        s_values_ = s_values_.mul(value);
      }
    });
